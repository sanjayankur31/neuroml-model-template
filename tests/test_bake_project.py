...
def test_bake_with_defaults(cookies):
    with bake_in_temp_dir(cookies) as result:
        ...
        assert "code" in found_toplevel_files
...
def test_bake_and_run_tests(cookies):
    with bake_in_temp_dir(cookies) as result:
        assert result.project.isdir()
        model_code_path = result.project / Path("code/model/")
        print(model_code_path)
        model_file = list(Path(model_code_path).glob("*.py"))[0]
        run_inside_dir(f"python {model_file} --help", str(model_code_path)) == 0
